...
The PCA plot revealed **clear separation of patient-derived neuronal lines from control samples**, with each patient cluster showing distinct displacement along the first two principal components. Notably, the **CRISPR-corrected line (CRISPR P1)** grouped closer to controls, suggesting a **partial transcriptomic rescue effect** following correction.
...
The visualization of the "Patient1 vs. CRISPR_P1" comparison revealed a massive and coherent transcriptional dysregulation (see Figures 2.2A and 2.2B).
...
The purpose of this phase is to "mine" our significant gene list for these non-coding players. We hypothesize that the pathogenic cascade is not limited to TFs but also involves the dysregulation of a non-coding regulatory network.

#### 7.1.1 Methods

The analysis was performed on the "clean" and statistically powerful isogenic dataset (Phase 2).

1.  **Candidate Identification:** We first identified all significant DEGs (`iso_sig_genes_df`). We then filtered this list to find "unmappable" genes—those with a valid Ensembl ID but *no* corresponding Entrez ID in the `org.Hs.eg.db` annotation package. This is a robust strategy for enriching for non-coding, uncharacterized, or novel genes.

2.  **Annotation:** This list of "unmappable" IDs (`iso_unmappable_ids`) was then used to query the comprehensive, up-to-date Ensembl database via `biomaRt`. We retrieved the official gene name, `gene_biotype`, and description for each ID.

3.  **Data Integration:** The `biomaRt` annotations were joined back to our `DESeq2` results (log2FoldChange, padj) to create a final table of all significant, unmapped genes.

4.  **lncRNA Filtering:** This final table was then filtered for `gene_biotype == "lncRNA"` to create our definitive list, which was saved as `novel_significant_lncRNAs.csv` and displayed as an interactive table.

```{r}
#| label: lncRNA
# 7.1 : Non-Coding Gene analysis
...
The analysis was highly successful and uncovered a significant non-coding component to the disease signature. The console output indicated a large pool of `iso_unmappable_ids` that were subsequently annotated.

The final filtered list, `novel_significant_lncRNAs.csv` (Table 7.1), provides a ranked list of the most significantly dysregulated lncRNAs.

-   By far, the top hit is **`DLX6-AS1` (DLX6 Antisense RNA 1)**, which is massively **up-regulated** (log2FoldChange: +3.80) and exceptionally significant (padj: 2.35e-27).

-   Other significant hits include `LINC01896` (down-regulated) and `SNHG14` (up-regulated).

#### 7.1.3 Interpretation

This is a major mechanistic discovery that adds a new, sophisticated layer to our model. The identification of `DLX6-AS1` as the #1 most significant lncRNA is not a random finding.

-   **A Plausible Mechanism:** The `DLX` family of transcription factors (`DLX1`, `DLX2`, `DLX5`, `DLX6`) are master regulators of neurodevelopment, particularly for GABAergic interneurons. Our previous heatmap (Figure 2.3) showed that `DLX1` and `DLX2` were among the top **down-regulated** protein-coding genes.

-   **A New Regulatory Axis:** `DLX6-AS1` is an *antisense* lncRNA to the `DLX6` gene. Such lncRNAs are known to regulate their sense-strand partners. This finding provides a powerful and elegant hypothesis for the `DLX` family's repression:

    1.  The *DNAJC6* mutation causes the lncRNA **`DLX6-AS1`** to be pathologically **up-regulated**.

    2.  This antisense lncRNA, in turn, likely **represses** the expression of the `DLX` family of TFs.

    3.  The repression of these master TFs leads to the observed **collapse of neurodevelopmental programs**.

This moves our model beyond a simple TF-gene relationship. It suggests a more complex **lncRNA-TF regulatory axis** is at the heart of the pathogenic cascade. This discovery provides the perfect rationale for the next step: to build a co-expression heatmap to visually test the relationship between this lncRNA and its predicted targets.

### **7.2 : Co-expression Heatmap**

The analysis in Phase 7.1 was a major breakthrough, identifying a novel, non-coding component of the disease signature. Specifically, it showed that the lncRNA **`DLX6-AS1`** is one of the most significantly **up-regulated** genes in the patient iPSCs.

This immediately generated a powerful hypothesis. We know from our "Core Signature" (Phase 4.5) that the key neurodevelopmental TFs **`DLX1`** and **`DLX2`** are strongly **down-regulated**.

The purpose of this analysis is to *statistically* and *visually* test this hypothesis. We will create a co-expression heatmap to determine if there is a correlation between the up-regulated `DLX6-AS1` and the down-regulated `DLX` family. We also included other key TFs (`EGR1`, `ATF4`) and disease-relevant genes (`DNAJC6`, `SNCA`, `CLTC`) to understand how this lncRNA fits into the broader pathogenic network.

#### 7.2.1 Methods

A master list of genes was compiled, including the top 5 lncRNAs (e.g., `DLX6-AS1`), the `DLX` family (`DLX1`, `DLX2`, `DLX5`, `DLX6`), other key TFs (`EGR1`, `ATF4`, `PAX6`, etc.), and core disease genes (`DNAJC6`, `SNCA`, `CLTC`).

The VST-normalized expression data for this curated list was extracted across *all 15 samples*. A pairwise Pearson correlation matrix was then calculated using the `cor()` function. This matrix, which shows the correlation coefficient (r) between every gene and every other gene, was plotted using the `pheatmap` package. Red cells indicate a positive correlation, while blue cells indicate a negative correlation.

```{r}
#| label: lncRNA Heatmap
...
The correlation heatmap (Figure 7.2) provides a stunning visual confirmation of our hypothesis and reveals that `DLX6-AS1` is a central hub in the pathogenic network.

-   **Hypothesis Validated (DLX Family):** The heatmap shows a profound, strong **negative correlation** between `DLX6-AS1` and its key target TFs:

    -   `DLX6-AS1` vs. `DLX1`: **r = -0.86**

    -   `DLX6-AS1` vs. `DLX2`: **r = -0.87**

    -   `DLX6-AS1` vs. `DLX5`: **r = -0.74**

    -   `DLX6-AS1` vs. `DLX6`: **r = -0.75**

-   **Link to Causal Gene:** This lncRNA is also strongly *negatively* correlated with the causal gene, **`DNAJC6` (r = -0.89)**.

-   **Link to Broader Neurodevelopment:** `DLX6-AS1` is negatively correlated with the *entire* down-regulated neurodevelopmental program, including **`EGR1` (r = -0.84)**, **`PAX6` (r = -0.81)**, **`LMX1A` (r = -0.83)**, and **`NEUROD1` (r = -0.80)**.

-   **Link to Stress Response:** Conversely, `DLX6-AS1` shows a strong **positive correlation** with the *activated* stress-response TF **`ATF4` (r = 0.78)**.

> **Figure 7.2:** Pearson co-expression heatmap of top lncRNAs and key disease/developmental genes across all 15 samples. Red indicates positive correlation; blue indicates negative. Note the strong negative correlation (dark blue, e.g., -0.86) between the lncRNA `DLX6-AS1` and the entire `DLX` family of transcription factors.

#### 7.2.3 Interpretation

This analysis provides a powerful validation of the lncRNA discovery. The data strongly supports a "regulatory axis" model:

1.  **Primary Event:** The loss of `DNAJC6` (down-regulation) is strongly correlated with the **up-regulation** of `DLX6-AS1`.

2.  **Downstream Consequence:** This up-regulated lncRNA is, in turn, strongly *negatively* correlated with the **down-regulation** of the `DLX` family and the entire neurodevelopmental TF program (`EGR1`, `PAX6`, etc.).

This suggests that `DLX6-AS1` may act as a key *mediator* of the pathogenic cascade, linking the upstream `DNAJC6` mutation to the downstream collapse of the neuronal identity program.

Furthermore, the fact that `DLX6-AS1` is *also* positively correlated with the `ATF4` stress response makes it a critical hub. This lncRNA is linked to *both* of the major pathogenic mechanisms identified in Phase 5 (repression of neuronal identity and activation of cellular stress). This finding provides a sophisticated, multi-layered regulatory model that is now ready to be visualized in a final integrated network.

## PHASE 8

The preceding analyses have been a "divide and conquer" investigation. We have independently identified:

1.  **The "Core Signature"** of dysregulated target genes (Phase 4, e.g., `PAX6`, `DLX1`, `DNAJC6`).

2.  The **Upstream TFs** that are functionally repressed or activated (Phase 5, e.g., `EGR1`, `ATF4`).

3.  A novel **lncRNA regulatory axis** (Phase 7, e.g., `DLX6-AS1`).

The purpose of this final phase is to *synthesize* all these disparate findings into a single, unified "Grand Unified Theory." By visualizing all these players—TFs, lncRNAs, and target genes—in one integrated network, we can build a testable *hypothesis model* of the core pathogenic cascade.

### Methods

A network graph was constructed using the `igraph` and `ggraph` packages.

-   **Nodes (The Players):** Three categories of nodes were defined:

    1.  **TFs:** The top 7 significant TFs from the `decoupleR` analysis (e.g., `ATF4`, `EGR1`, `SOX2`).

    2.  **lncRNAs:** The top 5 significant lncRNAs from the `biomaRt` analysis (e.g., `DLX6-AS1`).

    3.  **Key Genes:** A curated list of 12 critical neurodevelopmental and disease-relevant genes (e.g., `DNAJC6`, `CLTC`, `PAX6`, `DLX1`).

-   **Edges (The Connections):** Two categories of edges were defined to connect these nodes:

    1.  **"TF Regulation" (Red Edges):** These represent *known, validated* biological connections, extracted by filtering the DoRothEA regulon (`iso_regulon_dorothea`) for links between our defined TFs and target genes.

    2.  **"Co-expression" (Gray Edges):** These represent *newly inferred* relationships, extracted from our Pearson correlation matrix (Phase 7.2) for any link between a lncRNA and a key gene with `|r| > 0.4`.

The final network was plotted using the Kamada-Kawai ('kk') layout algorithm to position nodes based on their connectivity.

```{r}
#| label: Regulatory network visualization
# PHASE 8: Regulatory Network Visualization
...
# Get Top TFs (from your 5.1 table)
top_tfs <- c("ATF4", "MYCN", "EGR1", "PRDM14", "SOX2", "NR2C2", "TFAP2C")
tf_nodes <- tibble(
  name = top_tfs,
  type = "TF",
  activity = c(5.580, 3.445, -6.359, -4.754, -4.384, 5.428, -3.546) # Activity scores from 5.1
)

# Get Top lncRNAs (from 7.1)
# We re-use 'iso_lncrna_hits_df'
top_lncrnas <- iso_lncrna_hits_df %>% 
  head(5) %>% 
...
# Get Key Genes (from your 7.2 heatmap)
key_genes <- c("DNAJC6", "SNCA", "CLTC", "PAX6", "FOXA2", "NEUROD1", "SYN1", "DLX1", "DLX2", "DLX5", "DLX6", "LMX1A")
gene_nodes <- tibble(
  name = key_genes,
  type = "Gene",
  activity = 0 # Placeholder
...
# Combine all nodes
all_nodes <- bind_rows(tf_nodes, lncrna_nodes, gene_nodes) %>%
  distinct(name, .keep_all = TRUE) # Ensure no duplicates

# --- 3. Define Your "Edges" (The Lines) ---

# Edge List 1: TF -> Target Gene connections
# We use the 'iso_regulon_dorothea' from 5.1
tf_edges <- iso_regulon_dorothea %>%
  filter(source %in% all_nodes$name & target %in% all_nodes$name) %>%
  dplyr::select(from = source, to = target, mor) %>%
  mutate(type = "TF Regulation")

# Edge List 2: lncRNA -> Target Gene connections (from heatmap)
# We re-use 'iso_cor_matrix' from 7.2
# We need to "melt" the matrix into a list
cor_df <- as.data.frame(as.table(iso_cor_matrix))
colnames(cor_df) <- c("from", "to", "correlation")

lncrna_edges <- cor_df %>%
  # Keep only connections that involve one of our lncRNAs
  filter(from %in% top_lncrnas & to %in% key_genes) %>%
  # Filter out weak correlations
  filter(abs(correlation) > 0.4) %>% # You can adjust this cutoff
  mutate(type = "Co-expression") %>%
  dplyr::select(from, to, correlation, type)
...
# Plot it!
ggraph(network_graph, layout = 'kk') + # 'kk' is a nice layout algorithm
  
  # Draw the edges (lines)
  geom_edge_link(
    aes(color = type), # Color by regulation vs. co-expression
    arrow = arrow(length = unit(2, 'mm')), 
    end_cap = circle(3, 'mm')
  ) + 
  scale_edge_color_manual(values = c("TF Regulation" = "#E64B35", "Co-expression" = "#4D4D4D")) +
  
  # Draw the nodes (dots)
  geom_node_point(aes(shape = type, size = 6, color = type)) +
  scale_shape_manual(values = c("TF" = 17, "lncRNA" = 15, "Gene" = 19)) +
  scale_color_manual(values = c("TF" = "#E64B35", "lncRNA" = "#00A087", "Gene" = "gray50")) +
  
  # Draw the labels
  geom_node_text(aes(label = name), repel = TRUE, size = 4.0, fontface = "bold") +
  
  labs(title = "Core Regulatory Network (TFs & lncRNAs)",
       subtitle = "Connecting top regulators to key disease genes") +
  theme_void() +
  theme(legend.position = "bottom")
```
...
The resulting graph (Figure 8.1) provides a clear, interpretable model of the core regulatory network.

-   **Node & Edge Visualization:** The plot successfully visualizes the three node types (TFs as red triangles, lncRNAs as green squares, and Genes as gray circles) and the two connection types (known TF regulation as red lines, inferred lncRNA co-expression as gray lines).

-   **Identification of Hubs:** The network structure is not random; it is organized around key "hubs." The visualization clearly identifies **`EGR1` (TF)** and **`DLX6-AS1` (lncRNA)** as two of the most critical regulatory hubs.

-   **Confirmation of Regulatory Axes:** The graph visually confirms our previous hypotheses:

    -   **The lncRNA Axis:** `DLX6-AS1` (green square) is a major hub with gray "Co-expression" lines pointing to the entire `DLX` family (`DLX1`, `DLX2`, `DLX5`, `DLX6`), `PAX6`, and other key genes, validating the correlational finding from Phase 7.2.

    -   **The TF Axis:** `EGR1` and `SOX2` (red triangles) are shown as hubs with red "TF Regulation" lines pointing to their known targets, validating the `decoupleR` findings from Phase 5.

> **Figure 8.1:** An integrated regulatory network showing the key players (Nodes) and their relationships (Edges). The network is organized around two main hubs: the TF **`EGR1`** (red triangle), which controls targets via known TF regulation (red lines), and the lncRNA **`DLX6-AS1`** (green square), which is co-expressed with a separate cluster of developmental genes (gray lines).

### Interpretation

This network visualization serves as the capstone of our mechanistic investigation, synthesizing all previous findings into a single, cohesive model. This model strongly suggests that the collapse of the "Core Signature" is not due to a single point of failure but is the result of a **"pincer attack"** from at least two distinct, parallel pathogenic mechanisms:

1.  **A Classic TF Failure:** The loss of `DNAJC6` function leads to the repression of key neurodevelopmental TFs like `EGR1`, which in turn fails to activate its cohort of target genes.

2.  **A Non-Coding Failure:** In parallel, the mutation causes the pathological up-regulation of the lncRNA `DLX6-AS1`, which is strongly correlated with the repression of the entire `DLX` family of TFs.

Both of these pathogenic arms converge on the same set of downstream target genes: the "Core Signature" of neurodevelopmental and synaptic components (`PAX6`, `DLX1`, etc.). This sophisticated, multi-layered failure—where both the TF and lncRNA regulatory networks are simultaneously dysregulated—is a powerful explanatory model for the severe, systemic collapse of neurodevelopment seen in this disease.
...
-   **Strategy 1 (Overlap):** The Venn diagram (Figure 9.1) *visually* suggests a large overlap of 957 genes. However, the Fisher's Exact Test (Table 9.1) on this overlap returned a **p-value of 0.988**. This proves that the 957-gene overlap is **not statistically significant** and is no better than what would be expected from random chance.
